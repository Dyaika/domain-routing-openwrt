...
dnsmasqfull() {
    if apk info | grep -q dnsmasq-full; then
        ...
        apk del dnsmasq
        APK_CACHE_DIR=/tmp/ apk add dnsmasq-full
...
add_getdomains() {
    ...
    if [ "$COUNTRY" == 'russia_inside' ]; then
        EOF_DOMAINS=DOMAINS=https://raw.githubusercontent.com/dyaika/allow-domains/main/Russia/inside-dnsmasq-nfset.lst
    elif [ "$COUNTRY" == 'russia_outside' ]; then
        EOF_DOMAINS=DOMAINS=https://raw.githubusercontent.com/dyaika/allow-domains/main/Russia/outside-dnsmasq-nfset.lst
    elif [ "$COUNTRY" == 'ukraine' ]; then
        EOF_DOMAINS=DOMAINS=https://raw.githubusercontent.com/dyaika/allow-domains/main/Ukraine/inside-dnsmasq-nfset.lst
    fi
